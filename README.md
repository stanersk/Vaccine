...
기능적 요구사항
1. 정부관리자가 백신접종가능일자별로 접종병원, 백신수량을 등록/수정/삭제한다.
2. 예약자는 접종일자, 접종병원을 선택하여 예약한다. 
3. 예약과 동시에 예약가능한 백신수량이 감소한다.
4. 예약이 확정되면 예약내역 메시지가 예약자에게 전달된다. 
5. 예약자는 예약을 취소할 수 있다.
6. 예약이 취소되면 백신수량이 증가하고, 취소내역 메시지가 예약자에게 전달된다. 
7. 예약자는 접종 예약정보(예약번호, 예약상태, 접종예정일자, 접종병원, 백신종류)를 조회할 수 있다.

비기능적 요구사항
1. 트랜잭션
   - 백신수량 감소가 되지 않은 건은 예약이 되지 않아야 한다.(Sync 호출)
2. 장애격리
   - 메시지 전달, 병원 접종 수량 등이 되지 않더라도 예약은 받을 수 있어야 한다.(Async event-driven)
...
![image](https://user-images.githubusercontent.com/487999/79684167-3ecd2f00-826a-11ea-806a-957362d197e3.png)
    ...
    - 예약이 확정되면 예약내역 메시지가 예약자에게 전달된다 (ok)
...
![image](https://user-images.githubusercontent.com/487999/79684170-47256a00-826a-11ea-9777-e16fafff519a.png)
    ...
    - 예약자는 접종 예약정보(예약번호, 예약상태, 접종예정일자, 접종병원, 백신종류)를 조회할 수 있다 ( View-green sticker 의 추가로 ok) 
